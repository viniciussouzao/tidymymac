...
Landing-page development requires **Node.js 22**, npm, and **ffmpeg**. Install the
locked dependencies before building or testing:
...
npm run site:test    # builds CSS, starts the site, and runs Playwright
```

On Linux and in CI, install Chromium and its system dependencies with
`npx playwright install --with-deps chromium` instead.

`site/assets/site.css` is generated by Tailwind and is intentionally not
versioned. Always regenerate it from `site-src/styles.css` before serving the
site or running browser tests.

The demo media is reproducible from the original GIFs in `docs/images/`. Keep
those GIFs as the source files, and regenerate the published H.264 videos and
first-frame WebP posters with:

```bash
ffmpeg -i docs/images/tui-demo.gif -c:v libx264 -crf 23 -pix_fmt yuv420p -movflags +faststart -an site/assets/tui-demo.mp4
ffmpeg -i docs/images/cli-demo.gif -c:v libx264 -crf 23 -pix_fmt yuv420p -movflags +faststart -an site/assets/cli-demo.mp4
ffmpeg -i docs/images/tui-demo.gif -frames:v 1 site/assets/tui-demo-poster.webp
ffmpeg -i docs/images/cli-demo.gif -frames:v 1 site/assets/cli-demo-poster.webp
```
