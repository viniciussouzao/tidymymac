...
Scan roots are also resolved through symlinks before use (`resolveScanRoots`). That is not cosmetic: macOS ships `/tmp` as a symlink to `private/tmp`, and `filepath.WalkDir` only `Lstat`s its root — handed a symlink it reports that symlink as the single entry and never descends. `"/tmp"` as a scan root therefore produced exactly one candidate, `/tmp` itself, and nothing beneath the most security-relevant directory the Temp cleaner covers was ever scanned, reviewed or cleaned. (`/var/tmp` escaped this because only the *final* component is `Lstat`ed, and `tmp` there is a real directory.) The visible consequence is that entries are reported under their resolved spelling — `/private/tmp/...`, `/private/var/log/...`. `protected_paths` is unaffected: `internal/config` stores each protected entry under its literal spelling, its firmlink alias, and its `EvalSymlinks` resolution.
